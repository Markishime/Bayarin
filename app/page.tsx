...
import {
  ...
  Landmark, Languages, LoaderCircle, LockKeyhole, LogOut, Mail, Moon, MoreHorizontal, Pencil, Plus, ReceiptText,
  RefreshCw, Router, Settings, ShieldCheck, Smartphone, Sun, Tv, Upload,
  UserRound, UserPlus, WifiOff, X, Zap,
} from 'lucide-react';
...
function Welcome({ go }: { go: (s: Screen) => void }) {
  return <div className="welcome-screen">
    ...
    <div className="welcome-actions"><Button onClick={() => go('onboarding')} className="primary-button light">Magsimula <ChevronRight /></Button><button className="login-link" onClick={() => go('login')}>May account na? <b>Mag-login</b></button></div>
  </div>;
}
...
function Onboarding({ go, finish }: { go: (s: Screen) => void; finish?:()=>void }) {
  ...
  return <div className="onboarding-screen auth-onboarding">
    ...
    <div className={`onboarding-visual visual-${page.kind}`}>
      {page.kind==='calendar'&&<><div className="mini-calendar"><span>SEPTEMBER</span><b>08</b><i><Check /></i></div><span className="float-card bill"><Zap /> MERALCO</span></>}
      {page.kind==='reminder'&&<><div className="reminder-visual"><Bell/><i>2</i><span>Due soon</span></div><span className="float-card bill"><Clock3/> 3 days before</span></>}
      {page.kind==='secure'&&<><div className="secure-visual"><ShieldCheck/><span>Protected household data</span></div><span className="float-card bill"><ArrowUpRight/> Pay externally</span></>}
    </div>
...
function AuthShell({ title, subtitle, go, children }: { title:string;subtitle:string;go:(s:Screen)=>void;children:ReactNode }) {
  return <div className="auth-screen"><header className="auth-header"><button onClick={()=>go('welcome')} aria-label="Back to welcome"><ArrowLeft/></button><BrandMark small/><span/></header><div className="auth-copy"><span className="auth-kicker">BAYARIN ACCOUNT</span><h1>{title}</h1><p>{subtitle}</p></div>{!isSupabaseConfigured&&<div className="auth-config"><Info/><span><b>Connection required</b>{supabaseSetupMessage}</span></div>}{children}<p className="auth-trust"><ShieldCheck/> Secured by Supabase Auth. Bayarin never asks for wallet or bank passwords.</p></div>;
}
...
function LoginScreen({ go, onAuthenticated }: { go:(s:Screen)=>void;onAuthenticated:(user:User)=>void }) {
  const [email,setEmail]=useState('');const [password,setPassword]=useState('');const [error,setError]=useState('');const [loading,setLoading]=useState(false);
  const submit=async(e:FormEvent)=>{e.preventDefault();setError('');if(!supabase){setError(supabaseSetupMessage);return}setLoading(true);const {data,error:authError}=await supabase.auth.signInWithPassword({email:email.trim(),password});setLoading(false);if(authError){setError(readableAuthError(authError));return}if(data.user)onAuthenticated(data.user)};
...
function HomeScreen({ go, copy, userName }: { go: (s: Screen) => void; copy: { hello: string; due: string; unpaid: string }; userName:string }) {
  return <>
    <header className="home-header">
      <div className="avatar" aria-hidden="true">{userName.split(' ').map(part=>part[0]).join('').slice(0,2).toUpperCase()}</div><div className="greeting"><span>{copy.hello}</span><strong>{userName}</strong></div>
...
function SettingsScreen({ go, dark, lang, userName, email, onLogout }: { go:(s:Screen)=>void; dark:boolean; lang:Lang;userName:string;email:string;onLogout:()=>void }) {
  ...
  return <><AppHeader title="Settings"/><ScrollScreen className="with-nav"><section className="profile-card"><div className="avatar large">{initials}</div><div><b>{userName}</b><span>{email}</span></div><button aria-label="Edit profile"><Pencil/></button></section><SettingsGroup title="Preferences"><Setting icon={<Languages/>} label="Language" value={lang} onClick={()=>go('language')}/><Setting icon={dark?<Moon/>:<Sun/>} label="Appearance" value={dark?'Dark':'Light'} onClick={()=>go('appearance')}/><Setting icon={<Bell/>} label="Notifications" value="On" onClick={()=>go('notifications')}/></SettingsGroup><SettingsGroup title="Your data"><Setting icon={<ShieldCheck/>} label="Privacy & security" value="Supabase RLS" onClick={()=>go('privacy')}/><Setting icon={<History/>} label="Household activity" onClick={()=>go('activity')}/></SettingsGroup><SettingsGroup title="Preview states"><Setting icon={<WifiOff/>} label="Offline state" onClick={()=>go('offline')}/><Setting icon={<CircleHelp/>} label="Empty & error states" onClick={()=>go('empty')}/></SettingsGroup><button className="logout-button" onClick={onLogout}><LogOut/> Log out</button><section className="settings-note"><BrandMark small/><div><b>Bayarin</b><span>Version 1.1 · Made for Filipino households</span></div></section></ScrollScreen><BottomNav screen="settings" go={go}/></>;
}
...
export default function BayarinApp() {
  ...
  const logout=async()=>{if(supabase)await supabase.auth.signOut();setSession(null);setProfileReady(false);setScreen('welcome')};
  ...
  const visibleScreen=!session&&protectedScreens.includes(screen)?'login':screen;

  let content:ReactNode;
  switch(visibleScreen){
    case 'welcome':content=<Welcome go={setScreen}/>;break;case 'onboarding':content=<Onboarding go={setScreen} finish={session?completeOnboarding:undefined}/>;break;
    case 'login':content=<LoginScreen go={setScreen} onAuthenticated={handleAuthenticated}/>;break;case 'signup':content=<SignupScreen go={setScreen} setAuthEmail={setAuthEmail} setEmailPurpose={setEmailPurpose} onAuthenticated={handleAuthenticated}/>;break;
    ...
    case 'home':content=<HomeScreen go={setScreen} copy={languages[lang]} userName={userName}/>;break;case 'bills':content=<BillsScreen go={setScreen} userId={session?.user.id||''}/>;break;
    case 'bill-detail':content=<BillDetail go={setScreen} setPayOpen={setPayOpen}/>;break;case 'add-bill':content=<BillForm go={setScreen} userId={session?.user.id||''}/>;break;case 'edit-bill':content=<BillForm go={setScreen} userId={session?.user.id||''} edit/>;break;
    case 'mark-paid':content=<MarkPaid go={setScreen} userId={session?.user.id||''}/>;break;case 'success':content=<Success go={setScreen}/>;break;case 'activity':content=<ActivityScreen go={setScreen}/>;break;
